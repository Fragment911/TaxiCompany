        ...
        <div class="col-lg-12">
            <form role="form" action="${contextPath}/car/update" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select path="account" name="account" class="form-control" id="account">
                        ...
                        <c:if test="${car.account != null}">
                            <option value="${car.account.id}">${car.account.login}</option>
                        </c:if>
                        <c:forEach var = "driver" items="${driverList}">
                            <c:if test="${driver.id != car.account.id}">
                                <option value="${driver.id}">${driver.login}</option>
                            </c:if>
                            <c:if test="${driver.id == car.account.id}">
                                <option value="${driver.id}" selected>${driver.login}</option>
                            </c:if>
                        </c:forEach>
                    </select>
                </div>
                ...
                <table class="table table-striped table-dark">
                    ...
                    <tbody>
                    <c:forEach var = "option" items="${otherOptionList}">
                        <tr>
                            <td>${option.text}</td>
